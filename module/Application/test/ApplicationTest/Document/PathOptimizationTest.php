...
class OrderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testPath()
    {
        ...
        $tos = $veh->getPathReferences('getParent.getLocation.getTransportorder');
        $origins = [];
        $dest = [];
        $i = 0;
        $locations = $veh->getPathReferences('getParent.getLocation');
        
        foreach ($locations as $location) {
            if (isset($location->latitude) && strlen($location->latitude) > 1) {} else {
                
                $location->calculateGeoCode();
            }
            $mainOrigin = $location->latitude . "," . $location->longitude;
            $origins[0] = $location->latitude . "," . $location->longitude;
            foreach ($tos as $to) {
                if (isset($to->latitude) && strlen($to->latitude) > 1) {} else {
                    $to->calculateGeoCode();
                }
                $origins[$i + 1] = $to->latitude . "," . $to->longitude;
                $dest[$i] = [];
            ...
            $retM = $veh->getDistanceMatrix($origins, $dest ,'direct');
            $optR = $veh->getCompleteOptimalRoute($mainOrigin, $retM, "duration", 1000);
        }
    }
